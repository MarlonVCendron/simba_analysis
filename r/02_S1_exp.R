...
r_data <- read.csv("data/spss/S1_exp.csv", header=TRUE, na.strings = "NA", sep=";", dec=",")

data <- r_data %>%
  select(Video, GRUPO, A, B, C, D) %>%
  pivot_longer(cols = c(A, B, C, D), names_to = "Object", values_to = "Duration") %>%
  mutate(
...
ggsave("figures/glmm/S1_exp_duration_boxplot.png", p1, width = 8, height = 6, dpi = 300)

ggsave("figures/glmm/S1_exp_duration_qq.png", p2, width = 6, height = 6, dpi = 300)
